...
def test_parse_happy_path_returns_published(monkeypatch):
    # Patch service provider to avoid touching AWS/OpenAI
    import presentation.api.ingestion_router as router_mod

    monkeypatch.setattr(router_mod, "get_ingestion_service", lambda: _FakeService())

    client = TestClient(main.app)
    resp = client.post("/parse", json={"email_body": "hi", "sender": "a@b.com"})
